...
class MY_Model extends CI_Model
{
    ...
    public function set_soft_delete($type =true) {
        $this->soft_deletes = $type;
    }
            ...
            foreach($options as $option)
            {
                if(isset($this->{$option}) && !empty($this->{$option}))
                {
                    ...
                    foreach($this->{$option} as $key => $relation)
                    {
                        ...
                        $foreign_model_name = $foreign_model;
                        //dd($foreign_model_name);
    ...
    protected function add_created($row)
    {
        if($this->timestamps === TRUE || is_array($this->timestamps))
        {
            if(is_object($row) && !isset($row->{$this->_created_at_field}))
            {
                ...
                $row->{$this->_updated_at_field} = date('Y-m-d H:i:s');// new added
            }
    ...
    public function search($params = array(),$backtick=true){
        ...
        if(array_key_exists("returnType",$params) && $params['returnType'] == 'count'){
            ...
            if(array_key_exists('resultType', $params) && $params['resultType'] == 'class_object') {

                $result = ($query->num_rows() > 0)? $query->row(0,get_class($this)):FALSE;

            }else if(array_key_exists('resultType', $params) && $params['resultType'] == 'std_object'){
            ...
            if(array_key_exists('resultType', $params) && $params['resultType'] == 'class_object') {
                $result = ($query->num_rows() > 0)?$query->result(get_class($this)):FALSE;
